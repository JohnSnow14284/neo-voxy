...
public class VoxyNeoForgeConfig {
    ...
    /**
     * Sync values from the canonical Voxy JSON config into NeoForge's in-memory config view.
     *
     * Sodium's video-settings page writes to {@code voxy-config.json}.  On the next launch
     * NeoForge fires a Loading event for {@code voxy-client.toml}; if we copy TOML values into
     * {@link VoxyConfig#CONFIG} at that point, the TOML defaults overwrite the settings the user
     * just saved in-game.  Therefore startup sync must flow JSON -> NeoForge, not the other way
     * around.
     */
    private static void syncFromVoxyConfig() {
        ...
        SECTION_RENDER_DISTANCE.set(Math.round(VoxyConfig.CONFIG.sectionRenderDistance));
        ...
        // RenderStatistics is a runtime-only NeoForge setting and is intentionally not stored
        // in voxy-config.json.
     ...
     *
     * This is only used for explicit NeoForge config reloads, so changes made through the Mods
     * config screen can still be written back to Voxy's JSON config.
     */
